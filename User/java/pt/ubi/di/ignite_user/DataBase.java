...
public class DataBase extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "Ignite_user.db";
    ...
    //Método para adicionar um User
    public boolean addUser(String name, String surname, int dia, int mes, int ano, String username, String password ){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contenvalues = new ContentValues();
        //preparar o conteúdo a inserir
        contenvalues.put(COLUMN_0_1,name);
        contenvalues.put(COLUMN_0_2,surname);
        contenvalues.put(COLUMN_0_3,dia);
        contenvalues.put(COLUMN_0_4,mes);
        contenvalues.put(COLUMN_0_5,ano);
        contenvalues.put(COLUMN_0_6,username);
        contenvalues.put(COLUMN_0_7,password);
        //---------------------------------
        long result = db.insert(TABLE_USERS,null,contenvalues); //insere na base de dados
        //Se falhou a inserir
        if(result==-1){
            db.close();
            return false;
    ...
    public boolean editUsers(String id, String name, String surname, int dia, int mes, int ano, String username, String password){
        SQLiteDatabase db = this.getWritableDatabase();
        String[] nome = {id};
        //-------------------------------------------------//
        ContentValues contenvalues = new ContentValues();
        ...
        contenvalues.put(COLUMN_0_6,username);
        contenvalues.put(COLUMN_0_7,password);
        db.update(TABLE_USERS, contenvalues, COLUMN_0_6 + "=?", nome); //update na database
        db.close();
        return true;
    }

    //Método para adicionar um UserEvent
    public boolean addUserEvent(String name, String event){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contenvalues = new ContentValues();
        //preparar o conteúdo a inserir
        contenvalues.put(COLUMN_3_2,name);
        contenvalues.put(COLUMN_3_3,event);
        //---------------------------------
        long result = db.insert(TABLE_USERS_EVENTS,null,contenvalues); //insere na base de dados
        //Se falhou a inserir
        if(result==-1){
            db.close();
            return false;
    ...
    //Método para apagar um UserEvent
    public boolean eraseUserEvent(String name,String event){
        SQLiteDatabase db = this.getWritableDatabase();
        String[] id = {name,event};
        long result=db.delete(TABLE_USERS_EVENTS, COLUMN_3_2 + "=? AND "+COLUMN_3_3 + "=?",id); //update na database
        if(result==-1){
            db.close();
            return false;
        }
    ...
    //Método para retornar um cursor para a tabela de notícias
    ...
    //Método para retornar um cursor para a tabela de users
    ...
    //Método para retornar um cursor para a tabela de usersEvents
    public Cursor getUsersEvents(){
        SQLiteDatabase db = this.getWritableDatabase();
        String query ="SELECT * FROM UsersEvents";
        Cursor cursor = db.rawQuery(query,null);
        return cursor;
    }
}
